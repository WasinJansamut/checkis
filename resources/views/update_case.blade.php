    ...
    <div class="container-fluid">
        ...
        <form method="post" action="{{ route('submit_new_case') }}">
            ...
            <div class="mb-3">
                ...
                <label>name</label>
                <textarea class="form-control mb-3" placeholder="Confirm Password" name="name" rows="3" required>{{ $case->name }}</textarea>

                <label>คำอธิบาย</label>
                <textarea class="form-control mb-3" name="description" rows="3">{{ $case->description }}</textarea>

                <label>ตัวแปรที่ใช้ตรวจสอบ</label>
                <textarea class="form-control mb-3" name="check_fields" placeholder="เช่น id, name, age" rows="3">{{ $case->check_fields }}</textarea>

                <label>errorType</label><br>
                <select class="custom-select form-control select2" name="error_type" type="text"
                    required>
